...
describe 'Hangman class testing' do
  describe 'constructor' do
    args = { word: 'XXX', guess: 10, set: Set.new(%w[a b]) }
    emptyset = Set.new
    ...
    it 'assigns set correctly' do
      expect(x.letter_set).to eq(Set.new(%w[a b]))
    end
  ...
  describe 'Saving function' do
    args = { word: 'abc', guess: 99, set: Set.new(%w[z x y]) }
    x = HangmanGame.new(args)
    ...
    it 'stores the info as JSON string' do
      ...
      expect(f.read).to eq(JSON.dump({ word: 'abc', guess: 99, set: Set.new(%w[z x y]).to_a }))
    end
  end
  ...
  describe 'Generating words' do
    ...
    xit 'Considers only words <12 or >12 characters long' do
      ...
      expect(array).to eq(%w[this_one that_one])
    end
  ...
  describe 'to string method' do
    args = { word: 'abc', guess: 99, set: Set.new(%w[a x c]) }
